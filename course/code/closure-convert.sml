     ...
     of Cps.LetVal(x, v, t) =>
      ...
      | Cps.Case (x, k1, k2) =>
        let val x1 = freshVal()
            ...
            val env1 = freshEnv()
            val env2 = freshEnv()
        in (Closure.Case{cond=x, thenarg=x1
    ...
    case t
     of Closure.LetVal{name
      ...
      | Closure.Case {cond, thenarg, thenn, elsearg, elsee} =>
        (*guaranteed in freeVars() that t1= k1 x1, t2= k2 x2*)
        Closure.Case {cond=cond, thenarg=thenarg, thenn=convert thenn
                    , elsearg=elsearg, elsee=convert elsee}
      | Closure.LetPrim {name, opr, args, body} =>
        Closure.LetPrim {name=name, opr=opr, args=args, body=convert body}
      | Closure.If0 {cond, thenn=Closure.ContApp{cont, env, arg}, elsee} =>
        (*guaranteed in freeVars() that t1= k1 x1, t2= k2 x1*)
        Closure.LetVal{name=arg, value=Closure.Empty,
                       body=Closure.If0 {cond=cond
                                  , thenn=convert (Closure.ContApp{cont=cont
                                                                 , env=env, arg=arg})
                                  , elsee=convert elsee}}
