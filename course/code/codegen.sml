             ...
             of 0 => raise Fail "bug"
              | 1 => Machine.Block.Block{bindings =[]
                                       , exp = Machine.Block.Call {
                                         func = func
                                       , arg = hd args}}
              | _ => let val newVar = Temp.freshVal ()
                     in  Machine.Block.Block{
                         bindings = 
                         Machine.Binding.Bind{var=newVar
                                            , binding =
                                              Machine.Binding.AllocTuple{numFields = size}}
    ...
    case b
     of Flat.Binding.Empty => 
      ...
      | Flat.Binding.Prim (operator, xs) =>
        (Machine.Binding.Bind {var = x
                             , binding = Machine.Binding.Prim (transOperator operator, xs)})::acc
